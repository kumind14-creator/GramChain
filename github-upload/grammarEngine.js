...
const wordList = [
  ...
  { id: "joahada", lemma: "좋아하다", stem: "좋아하", pos: "verb", meaningRu: "любить / нравиться", level: "A1", topic: "feelings", isCommon: true, blockedGrammarIds: ["go_sipda", "neuryeogo_hada"] },
  { id: "silheohada", lemma: "싫어하다", stem: "싫어하", pos: "verb", meaningRu: "не любить / не нравиться", level: "A2", topic: "feelings", isCommon: true, blockedGrammarIds: ["go_sipda", "neuryeogo_hada"] },
  ...
  { id: "datda", lemma: "닫다", stem: "닫", pos: "verb", meaningRu: "закрывать", level: "A1", topic: "home", isCommon: true, blockedGrammarIds: ["go_sipda", "neuryeogo_hada"] },
...
Object.assign(exampleBank.chain, {
  ...
  ),  geot_gatda_adj__ji_anta: makeCuratedExample(
    "겉으로는 괜찮은 것 같지만 사실은 그렇게 쉽지 않아요.",
    "Снаружи может казаться, что все в порядке, но на самом деле это не так уж легко."
  ),
  geot_gatda_verb_present__ji_anta: makeCuratedExample(
    "창밖을 보니까 아직은 비가 오는 것 같지 않아요.",
    "Судя по окну, пока не похоже, что идет дождь."
  ),
  ...
  ),  ji_anta__past_tense__jiman: makeCuratedExample(
    "어제는 바쁘지 않았지만 오늘은 회의가 많아서 정신이 없어요.",
    "Вчера было не очень занято, но сегодня из-за множества встреч совсем нет передышки."
  ),
  ji_anta__past_tense__eul_ttae: makeCuratedExample(
    "몸이 아프지 않았을 때는 매일 저녁 공원에서 산책했어요.",
    "Когда я не болел(а), я каждый вечер гулял(а) в парке."
  ),
  ji_anta__past_tense__myeon_eumyeon: makeCuratedExample(
    "어제처럼 피곤하지 않았으면 오늘 수업이 더 쉬웠을 거예요.",
    "Если бы я сегодня не был(а) таким уставшим(ей), как вчера, урок прошел бы легче."
  ),
  ...
  ),  go_sipda__ji_anta__past_tense: makeCuratedExample(
    "어제는 속이 안 좋아서 아무것도 먹고 싶지 않았어요.",
    "Вчера у меня болел живот, поэтому мне совсем не хотелось ничего есть."
  ),
  go_sipda__ji_anta__aseo_eoseo: makeCuratedExample(
    "오늘은 너무 피곤해서 아무 말도 하고 싶지 않아서 먼저 집에 갈게요.",
    "Сегодня я так устал(а), что совсем не хочу разговаривать, поэтому пойду домой пораньше."
  ),
  go_sipda__ji_anta__jiman: makeCuratedExample(
    "지금은 아무것도 먹고 싶지 않지만 저녁에는 같이 식사할 수 있어요.",
    "Сейчас мне не хочется ничего есть, но вечером я смогу поужинать вместе со всеми."
  ),
  go_sipda__ji_anta__eul_ttae: makeCuratedExample(
    "주말에 밖에 나가고 싶지 않을 때는 집에서 영화만 봐요.",
    "Когда на выходных мне не хочется выходить из дома, я просто смотрю фильмы дома."
  ),
  go_sipda__ji_anta__myeon_eumyeon: makeCuratedExample(
    "내일도 아무것도 먹고 싶지 않으면 병원에 가 보는 게 좋겠어요.",
    "Если и завтра тебе совсем не будет хотеться есть, лучше сходить в больницу."
  ),  go_sipda__geot_gatda_adj__ji_anta: makeCuratedExample(
    "요즘은 너무 피곤해서 누구도 만나고 싶은 것 같지 않아요.",
    "В последнее время я так устаю, что, кажется, совсем не хочу ни с кем встречаться."
  ),
  go_sipda__geot_gatda_adj__aseo_eoseo: makeCuratedExample(
    "오늘은 집에 가고 싶은 것 같아서 약속을 다 취소했어요.",
    "Кажется, сегодня мне очень хочется домой, поэтому я отменила все встречи."
  ),
  go_sipda__geot_gatda_adj__jiman: makeCuratedExample(
    "민수는 지금 쉬고 싶은 것 같지만 회의가 아직 안 끝났어요.",
    "Похоже, Минсу сейчас хочет отдохнуть, но собрание еще не закончилось."
  ),
  go_sipda__geot_gatda_adj__eul_ttae: makeCuratedExample(
    "집에 가고 싶은 것 같을 때는 잠깐 밖에 나가서 바람을 쐬어요.",
    "Когда мне кажется, что очень хочется домой, я ненадолго выхожу подышать воздухом."
  ),
  go_sipda__geot_gatda_adj__myeon_eumyeon: makeCuratedExample(
    "정말 쉬고 싶은 것 같으면 오늘은 일찍 자는 게 좋겠어요.",
    "Если тебе правда кажется, что очень хочется отдохнуть, лучше сегодня лечь спать пораньше."
  ),
  ...
  ),  neuryeogo_hada__ji_anta__aseo_eoseo: makeCuratedExample(
    "오늘은 밖에 나가려고 하지 않아서 집에서 조용히 쉴 거예요.",
    "Сегодня я не собираюсь никуда выходить, поэтому спокойно отдохну дома."
  ),
  neuryeogo_hada__ji_anta__jiman: makeCuratedExample(
    "주말에는 멀리 가려고 하지 않지만 근처 카페에는 들를 거예요.",
    "На выходных я не собираюсь ехать далеко, но зайду в кафе неподалеку."
  ),
  neuryeogo_hada__ji_anta__eul_ttae: makeCuratedExample(
    "저녁에 아무 일도 하려고 하지 않을 때는 음악만 들으면서 쉬어요.",
    "Когда вечером я уже ничего не собираюсь делать, я просто отдыхаю и слушаю музыку."
  ),
  neuryeogo_hada__ji_anta__myeon_eumyeon: makeCuratedExample(
    "내일도 일찍 일어나려고 하지 않으면 오늘은 알람을 끄고 자도 돼요.",
    "Если и завтра ты не собираешься вставать рано, сегодня можно лечь спать без будильника."
  ),
  neuryeogo_hada__geot_gatda_verb_present__ji_anta: makeCuratedExample(
    "민수는 오늘 학교에 가려고 하는 것 같지 않아요.",
    "Похоже, Минсу сегодня не собирается идти в школу."
  ),
...
function humanizeReason(reason, previousGrammarId, nextGrammarId) {
  ...
  if (reason === "Эти грамматики не соединяются в таком порядке.") {
    return `${previousLabel} не ставится перед ${nextLabel}.`;
  }
...
function explainBestFailedOrder(selectedGrammarIds) {
  ...
  for (const basePos of ["verb", "adjective"]) {
    for (const order of permutations) {
        ...
        if (index === 0) {
          if (!grammar.inputType.includes(basePos)) {
            failure = {
              builtOrder: [],
              blockedGrammarId: grammarId,
              reason: `${grammar.label} не ставится первой после ${basePos === "verb" ? "глагола" : "прилагательного"}.`,
              depth: 0
            };
            break;
          }
      ...
      if (!bestFailure || failure.depth > bestFailure.depth) {
        bestFailure = failure;
        blockedGrammarId = failure.blockedGrammarId || null;
      }
    }
  }
  ...
  const missingLabels = selectedGrammarIds
    .filter((grammarId) => grammarId === blockedGrammarId || !bestFailure.builtOrder.includes(grammarId))
    .map((grammarId) => getGrammarById(grammarId)?.label || grammarId)
    .join(", ");

  if (builtLabels && missingLabels) {
    return `Почти вся цепочка собирается, но ${missingLabels} не встраивается вместе с остальными. Без нее получается такая рабочая цепочка: ${builtLabels}. Дальше цепочка останавливается, потому что ${bestFailure.reason}`;
  }

  if (builtLabels) {
    return `Рабочая часть цепочки выглядит так: ${builtLabels}. Дальше цепочка останавливается, потому что ${bestFailure.reason}`;
  }
...
function explainNearMissSelection(selectedGrammarIds) {
  ...
  for (const pair of swapPairs) {
    ...
    for (const basePos of ["verb", "adjective"]) {
      ...
      return `Эта цепочка почти собирается. Здесь подходит ${correctGrammar?.label || pair.to}, а ${wrongGrammar?.label || pair.from} в эту точку не встраивается, потому что ${typeReason}. Подходящий порядок: ${orderLabels}.`;
    }
  }
...
function addAEOFamily(stem, conjugationType) {
  ...
  if (!hasBatchim(stem) && lastJung === "ㅣ") {
    return replaceLastJung(stem, 6);
  }
...
function addPast(stem, conjugationType) {
  ...
  if (!hasBatchim(stem) && lastJung === "ㅣ") {
    return addFinalConsonant(replaceLastJung(stem, 6), 20);
  }
...
function toPoliteAnswer(form, resultType, lastGrammarId) {
  ...
  // These forms connect a clause to what follows, so they do not take the polite ending -요.
  if (["jiman", "neunde", "aseo_eoseo", "myeon_eumyeon", "eul_ttae"].includes(lastGrammarId)) {
    return form;
  }
...
function generateExercise(selectedGrammarIds, preferredWordId, options = {}) {
  ...
  const grammarCount = Array.isArray(selectedGrammarIds) ? selectedGrammarIds.length : 0;
  ...
  for (const word of candidates) {
    ...
    if (grammarCount >= 2 && !isNaturalResolvedOrder(chainResult.order, grammarCount)) continue;
  ...
  for (const grammarCount of grammarCounts) {
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
      ...
      for (const word of candidateSet) {
        ...
        if (!exercise.ok) continue;
        if (!isNaturalResolvedOrder(exercise.resolvedOrder, grammarCount)) continue;
...
function checkExerciseAnswer(exercise, studentAnswer) {
  ...
  return {
    ...
    message: "Порядок верный, но сама форма собрана с ошибкой.",
    resolvedOrder: expectedOrder,
    correctAnswer: exercise.correctAnswer,
    explanation: exercise.explanation
  };
...
function generateStudentOptions() {
  return {
    selfPractice: {
      id: "self_practice",
      title: "Ученик выбирает грамматики сам",
    ...
    freePractice: {
      ...
      description: "Приложение само подбирает естественные цепочки.",
      difficultyOptions: [
        { id: "easy", label: "Полегче", description: "Обычно 2 грамматики." },
        { id: "hard", label: "Посложнее", description: "Обычно 4 грамматики, иногда 3." }
      ]
    }
  };
}
...
function getEncouragementMessage(errorType = "wrong_form") {
  const messageMap = {
    not_connectable: [
      "Это нормально: не все грамматики можно соединять. Давай посмотрим, какие варианты подойдут.",
      "Ничего страшного. Здесь просто недопустимое сочетание грамматик."
    ],
    wrong_order: [
      "Ты на правильном пути. Здесь нужно только переставить порядок.",
      "Хорошая попытка. Грамматики совместимы, но их нужно соединить в другом порядке."
    ],
    wrong_form: [
      "Почти получилось. Порядок ты уже поймал(а), осталось доточить форму.",
      "Это хорошая попытка. Связь между грамматиками ты увидел(а), теперь попробуй исправить окончание."
    ]
  };
...
function getSingleExample(grammarId) {
  return pickRandomExample(exampleBank.single[grammarId]);
}
...
function validateExampleQuality(example, context = {}) {
  const issues = [];
  const sentence = String(example?.sentence ?? "").trim();
  const meaningRu = String(example?.meaningRu ?? "").trim();
  const normalizedMeaning = normalizeQualityText(meaningRu);

  const weakRussianPatterns = [
    "сегодня это так",
    ...
    "это будет хорошо",
    "речь шла о том",
    "сейчас мне кажется, что это происходит",
    "состояние постепенно меняется"
  ];

  if (!sentence) {
    issues.push("missing_sentence");
  } else {
    if (!containsHangul(sentence)) {
      issues.push("sentence_has_no_hangul");
    }
    if (sentence.length < 8) {
      issues.push("sentence_too_short");
    }
  }
  ...
  if (!meaningRu) {
    ...
    if (meaningRu.length < 8) {
      issues.push("translation_too_short");
    }
    if (weakRussianPatterns.some((pattern) => normalizedMeaning.includes(pattern))) {
      issues.push("translation_too_generic");
    }
  }

  if (context.requireChainSpecificity && example?.sourceType === "generated" && normalizedMeaning) {
    if (
      !normalizedMeaning.includes("кажется") &&
      !normalizedMeaning.includes("если") &&
      !normalizedMeaning.includes("хот") &&
      !normalizedMeaning.includes("собира") &&
      !normalizedMeaning.includes("не ")
    ) {
      issues.push("generated_translation_not_specific_enough");
    }
  }
...
function pickBestLocalExample(exercise) {
  const grammarIds = Array.isArray(exercise?.resolvedOrder) ? exercise.resolvedOrder : [];
  const candidates = [];
  const isChain = grammarIds.length >= 2;

  if (isChain) {
    const chainExample = getChainExample(grammarIds);
    if (chainExample) {
      candidates.push({
        ...chainExample,
        matchType: "chain",
  ...
  const lastGrammarId = grammarIds[grammarIds.length - 1];
  if (!isChain && lastGrammarId) {
    const singleExample = getSingleExample(lastGrammarId);
    if (singleExample) {
      candidates.push({
        ...singleExample,
        matchType: "single",
  ...
  const wordMatchedExample = buildWordMatchedExampleForExercise(exercise);
  if (wordMatchedExample) {
    candidates.push({
      ...wordMatchedExample,
      matchType: isChain ? "word_matched_chain" : "word_matched_single",
      grammarIds
    });
  }

  const fallbackExample = buildFallbackExampleForExercise(exercise);
  if (fallbackExample && !isChain) {
    candidates.push({
      ...fallbackExample,
      matchType: isChain ? "generated_chain" : "generated_single",
      grammarIds
    });
  }
  ...
  const ranked = candidates
    .map((example) => {
      const quality = validateExampleQuality(example, {
        requireChainSpecificity: isChain
      });

      let priority = 0;
      if (example.matchType === "chain") priority += 40;
      if (example.matchType === "word_matched_chain") priority += 34;
      if (example.matchType === "generated_chain") priority += 22;
      if (example.matchType === "single") priority += 18;
      if (example.matchType === "word_matched_single") priority += 16;
      if (example.matchType === "generated_single") priority += 8;
      if (example.sourceType === "curated") priority += 20;
      if (exampleUsesExactAnswer(example, exercise)) priority += 38;
      if (example.sourceType === "generated") priority -= 5;
      priority -= quality.issues.length * 25;
...
function buildFallbackExampleForExercise(exercise) {
  ...
  const curatedThreeGrammarFallbacks = {
    go_sipda__geot_gatda_adj__past_tense: {
      ...
      meaningRu: `Вчера мне очень хотелось ${actionMeaningRu}.`
    },
    go_sipda__ji_anta__past_tense: {
      sentence: `어제 ${objectKo}${answer}.`,
      meaningRu: `Вчера мне совсем не хотелось ${actionMeaningRu}.`
    },
    ...
    neuryeogo_hada__geot_gatda_verb_present__past_tense: {
      sentence: `아까 ${objectKo}${answer}.`,
      meaningRu: `Еще недавно казалось, что я собираюсь ${actionMeaningRu}.`
    },
    ji_anta__geot_gatda_verb_present__past_tense: {
      sentence: `아까 ${objectKo}${answer}.`,
      meaningRu: `Еще недавно казалось, что я не буду ${actionMeaningRu}.`
    },
    ajida_eojida__geot_gatda_verb_present__past_tense: {
      sentence: `아까 ${objectKo}${answer}.`,
      meaningRu: "Еще недавно казалось, что ситуация постепенно меняется."
    }
  };
  ...
  const improvedFallbackMap = {
    past_tense: {
      sentence: `어제 ${objectKo}${answer}.`,
      meaningRu: `Вчера ${actionMeaningRu}.`
    },
    go_sipda: {
    ...
    aseo_eoseo: {
      ...
      meaningRu: "Сегодня так получилось, поэтому я, наверное, останусь дома и немного отдохну."
    },
    jiman: {
      sentence: `오늘은 ${objectKo}${answer} 괜찮아요.`,
      meaningRu: "Сегодня это действительно так, но ситуация не такая уж серьезная."
    },
    eul_ttae: {
      sentence: `${objectKo}${answer} 잠깐 쉬어요.`,
      meaningRu: "Когда так происходит, я обычно делаю короткий перерыв."
    },
    ji_anta: {
      sentence: `저는 오늘 ${objectKo}${answer}.`,
    ...
    geot_gatda_adj: {
      ...
      meaningRu: "Сейчас мне кажется, что дело обстоит именно так."
    },
    geot_gatda_verb_present: {
      sentence: `지금은 ${objectKo}${answer}.`,
      meaningRu: "Сейчас мне кажется, что это действительно происходит."
    },
    ajida_eojida: {
      sentence: `요즘은 ${objectKo}${answer}.`,
      meaningRu: "В последнее время правда заметно, что ситуация постепенно меняется."
    },
    myeon_eumyeon: {
      sentence: `내일 ${objectKo}${answer} 좋겠어요.`,
      meaningRu: "Если так получится, это будет очень кстати."
    }
  };
  ...
  const chainTranslationMap = {
    go_sipda__ji_anta__past_tense: `Вчера мне совсем не хотелось ${actionMeaningRu}.`,
  ...
  const fallbackMap = {
    past_tense: {
      sentence: `어제 ${objectKo}${answer}.`,
      meaningRu: `Вчера так получилось: пришлось ${actionMeaningRu}.`
    },
    go_sipda: {
      sentence: `저는 오늘 ${objectKo}${answer}.`,
      meaningRu: `Сегодня я очень хочу ${actionMeaningRu}.`
    },
    neuryeogo_hada: {
    ...
    aseo_eoseo: {
      ...
      meaningRu: `Сегодня так получилось, поэтому я останусь дома.`
    },
    jiman: {
      sentence: `오늘은 ${objectKo}${answer} 괜찮아요.`,
      meaningRu: `Сегодня это действительно так, но ситуация не такая уж серьезная.`
    ...
    eul_ttae: {
      ...
      meaningRu: `Когда так бывает, я обычно слушаю музыку.`
    },
    ji_anta: {
      sentence: `저는 오늘 ${objectKo}${answer}.`,
      meaningRu: `Сегодня мне не хочется ${actionMeaningRu}.`
    ...
    geot_gatda_adj: {
      ...
      meaningRu: "Кажется, все именно так и есть."
    },
    geot_gatda_verb_present: {
      sentence: `민수는 ${objectKo}${answer}.`,
      meaningRu: "Кажется, это сейчас действительно происходит."
    },
    ajida_eojida: {
      sentence: `요즘은 ${objectKo}${answer}.`,
      meaningRu: "В последнее время действительно чувствуется, что состояние меняется."
    },
    myeon_eumyeon: {
      sentence: `내일 ${objectKo}${answer} 좋아요.`,
      meaningRu: "Если так получится, это будет хорошо."
    }
  };
  ...
  if (!fallback) {
    return {
      sentence: answer,
      meaningRu: "Пример для этой формы пока дан в нейтральном виде.",
      sourceType: "generated"
    };
  }
...
function getBetterFallbackMeaningRu(exercise) {
  ...
  const genericMeaningMap = {
    ...
    jiman: "Сегодня это действительно так, но ситуация не такая уж серьезная.",
    ...
    geot_gatda_verb_present: "Сейчас мне кажется, что это действительно происходит.",
    ajida_eojida: "В последнее время правда заметно, что ситуация постепенно меняется.",
    myeon_eumyeon: "Если так получится, это будет очень кстати."
  };
...
function exampleUsesExactAnswer(example, exercise) {
  const sentence = normalizeExampleMatchText(example?.sentence);
  const answer = normalizeExampleMatchText(exercise?.correctAnswer);
...
function getWordMatchedPredicateRu(exercise) {
  const grammarIds = Array.isArray(exercise?.resolvedOrder) ? exercise.resolvedOrder : [];
  const actionMeaningRu = getFallbackUsageContext(exercise?.word).actionRu;

  const has = (grammarId) => grammarIds.includes(grammarId);

  if (has("go_sipda") && has("past_tense") && has("ji_anta")) {
    return `мне совсем не хотелось ${actionMeaningRu}`;
  }

  if (has("go_sipda") && has("past_tense")) {
    return `мне хотелось ${actionMeaningRu}`;
  }

  if (has("go_sipda") && has("ji_anta")) {
    return `мне не хочется ${actionMeaningRu}`;
  }

  if (has("go_sipda")) {
    return `мне хочется ${actionMeaningRu}`;
  }

  if (has("neuryeogo_hada") && has("past_tense") && has("ji_anta")) {
    return `я не собирался(ась) ${actionMeaningRu}`;
  }

  if (has("neuryeogo_hada") && has("past_tense")) {
    return `я собирался(ась) ${actionMeaningRu}`;
  }

  if (has("neuryeogo_hada") && has("ji_anta")) {
    return `я не собираюсь ${actionMeaningRu}`;
  }

  if (has("neuryeogo_hada")) {
    return `я собираюсь ${actionMeaningRu}`;
  }

  if (has("ji_anta") && has("past_tense")) {
    return `я не стал(а) ${actionMeaningRu}`;
  }

  if (has("ji_anta")) {
    return `я не хочу ${actionMeaningRu}`;
  }

  if (has("past_tense")) {
    return `мне пришлось ${actionMeaningRu}`;
  }

  return `речь идет о том, чтобы ${actionMeaningRu}`;
}

function buildWordMatchedExampleForExercise(exercise) {
  if (!exercise?.ok || !Array.isArray(exercise.resolvedOrder) || exercise.resolvedOrder.length === 0) {
    return null;
  }

  if (!exercise?.word || exercise.word.pos !== "verb") {
    return null;
  }
  ...
  const objectKo = usage.objectKo ? `${usage.objectKo} ` : "";
  const lastGrammarId = exercise.resolvedOrder[exercise.resolvedOrder.length - 1];
  const predicateRu = getWordMatchedPredicateRu(exercise);

  const sentenceMap = {
    past_tense: `어제 ${objectKo}${answer}.`,
    go_sipda: `저는 오늘 ${objectKo}${answer}.`,
    neuryeogo_hada: `저는 오늘 ${objectKo}${answer}.`,
    aseo_eoseo: `오늘은 ${objectKo}${answer} 집에서 조용히 쉴 거예요.`,
    jiman: `어제 ${objectKo}${answer} 결국 집에서 쉬었어요.`,
    eul_ttae: `${objectKo}${answer} 저는 잠깐 혼자 있고 싶어요.`,
    ji_anta: `저는 오늘 ${objectKo}${answer}.`,
    geot_gatda_adj: `지금은 ${objectKo}${answer}.`,
    geot_gatda_verb_present: `지금은 ${objectKo}${answer}.`,
    myeon_eumyeon: `내일 ${objectKo}${answer} 바로 연락해 주세요.`
  };

  const meaningMap = {
    past_tense: `Вчера ${predicateRu}.`,
    go_sipda: `Сегодня ${predicateRu}.`,
    neuryeogo_hada: `Сегодня ${predicateRu}.`,
    aseo_eoseo: `Сегодня ${predicateRu}, поэтому я немного отдохну дома.`,
    jiman: `Вчера ${predicateRu}, но потом планы изменились.`,
    eul_ttae: `Когда ${predicateRu}, мне хочется ненадолго побыть одной(одному).`,
    ji_anta: `Сегодня ${predicateRu}.`,
    geot_gatda_adj: `Сейчас кажется, что ${predicateRu}.`,
    geot_gatda_verb_present: `Сейчас кажется, что ${predicateRu}.`,
    myeon_eumyeon: `Если ${predicateRu}, пожалуйста, сразу скажи мне.`
  };

  const sentence = sentenceMap[lastGrammarId];
  const meaningRu = meaningMap[lastGrammarId];

  if (!sentence || !meaningRu) {
    return null;
  }
  ...
  return {
    ...
    meaningRu,
    sourceType: "generated",
    sourceLabel: "Word-matched example"
  };
}
...
function getExampleForExercise(exercise) {
  ...
  if (!Array.isArray(exercise.resolvedOrder) || exercise.resolvedOrder.length === 0) {
    return null;
  }

  const ranked = pickBestLocalExample(exercise);
  return ranked ? ranked.example : null;
}
...
function getAllAuditableGrammarOrders() {
  const singleOrders = grammarList.map((grammar) => [grammar.id]);
  return [
    ...singleOrders,
...
function auditExampleQuality() {
  const orders = getAllAuditableGrammarOrders();
  const issues = [];
  ...
  for (const order of orders) {
    ...
    if (!ranked.quality.ok) {
      issues.push({
        grammarIds: order,
        severity: "warning",
        sourceType: ranked.example.sourceType,
        matchType: ranked.example.matchType,
        sentence: ranked.example.sentence,
        meaningRu: ranked.example.meaningRu,
        issues: ranked.quality.issues
      });
    }
  }
...
function mockAppScreen(screenId, payload = {}) {
  ...
  if (screenId === "exercise") {
    ...
    return [
      "=== Упражнение ===",
      `Слово: ${exercise.word.lemma} (${exercise.word.meaningRu})`,
      `Грамматики: ${exercise.selectedGrammarIds.join(", ")}`,
      "",
      "Поле ответа: [________________]",
      "Кнопки: [Проверить] [Показать ответ]"
    ].join("\n");
  }
  ...
  if (screenId === "result") {
    ...
    return [
      "=== Результат ===",
      `Статус: ${checkResult?.ok ? "Правильно" : "Еще нет"}`,
      !checkResult?.ok && encouragement ? `Подбадривание: ${encouragement}` : null,
      `Правильный ответ: ${exercise?.correctAnswer ?? "-"}`,
      exercise?.explanation?.flow ? `Схема: ${exercise.explanation.flow}` : null,
      exercise?.explanation?.why ? `Почему: ${exercise.explanation.why}` : null,
      example?.sentence ? `Пример: ${example.sentence}` : null,
      example?.meaningRu ? `Перевод: ${example.meaningRu}` : null,
      example?.sourceLabel ? `Источник: ${example.sourceLabel}` : null
    ].filter(Boolean).join("\n");
  }
...
Object.assign(exampleBank.chain, {
  go_sipda__geot_gatda_adj__ji_anta: makeCuratedExample(
    "요즘은 너무 지쳐서 누구도 만나고 싶은 마음이 아닌 것 같아요.",
    "В последнее время я так выматываюсь, что, похоже, совсем не хочу ни с кем встречаться."
  ),
  neuryeogo_hada__geot_gatda_verb_present__aseo_eoseo: makeCuratedExample(
    "민수는 오늘 발표를 준비하려고 하는 것 같아서 도서관에 먼저 갔어요.",
    "Похоже, Минсу сегодня собирается готовить презентацию, поэтому он ушел в библиотеку пораньше."
  ),
  neuryeogo_hada__geot_gatda_verb_present__jiman: makeCuratedExample(
    "지수는 주말에 집에서 쉬려고 하는 것 같지만 친구들이 계속 불러요.",
    "Похоже, Чису собирается отдохнуть дома на выходных, но друзья все время зовут ее куда-то."
  ),
  neuryeogo_hada__geot_gatda_verb_present__eul_ttae: makeCuratedExample(
    "동생이 혼자 공부하려고 하는 것 같을 때는 방해하지 않아요.",
    "Когда кажется, что младший брат собирается заниматься один, я ему не мешаю."
  ),
  neuryeogo_hada__geot_gatda_verb_present__myeon_eumyeon: makeCuratedExample(
    "민수가 다시 이야기하려고 하는 것 같으면 조금 더 기다려 주세요.",
    "Если кажется, что Минсу собирается заговорить снова, пожалуйста, подождите еще немного."
  ),
  ajida_eojida__ji_anta__aseo_eoseo: makeCuratedExample(
    "밤이 되어도 날씨가 시원해지지 않아서 에어컨을 켰어요.",
    "Даже к вечеру погода не стала прохладнее, поэтому я включил(а) кондиционер."
  ),
  ajida_eojida__ji_anta__jiman: makeCuratedExample(
    "연습해도 발음이 바로 자연스러워지지 않지만 조금씩 나아지고 있어요.",
    "Даже если тренироваться, произношение не становится естественным сразу, но понемногу оно улучшается."
  ),
  ajida_eojida__ji_anta__eul_ttae: makeCuratedExample(
    "날씨가 쉽게 따뜻해지지 않을 때는 얇은 외투를 꼭 챙겨요.",
    "Когда погода никак не становится теплее, я обязательно беру с собой легкую куртку."
  ),
  ajida_eojida__ji_anta__myeon_eumyeon: makeCuratedExample(
    "통증이 며칠 안에 나아지지 않으면 병원에 다시 가 보세요.",
    "Если боль не станет слабее в ближайшие дни, сходите в больницу еще раз."
  ),
  ajida_eojida__geot_gatda_verb_present__ji_anta: makeCuratedExample(
    "약을 먹었는데도 감기가 빨리 나아지는 것 같지 않아요.",
    "Даже после лекарства не похоже, что простуда быстро проходит."
  ),
  ajida_eojida__geot_gatda_verb_present__aseo_eoseo: makeCuratedExample(
    "밖이 점점 어두워지는 것 같아서 서둘러 집에 돌아왔어요.",
    "Показалось, что на улице быстро темнеет, поэтому я поспешил(а) вернуться домой."
  ),
  ajida_eojida__geot_gatda_verb_present__jiman: makeCuratedExample(
    "요즘은 일이 조금 쉬워지는 것 같지만 아직 실수도 많아요.",
    "Сейчас работа как будто становится немного легче, но ошибок у меня еще много."
  ),
  ajida_eojida__geot_gatda_verb_present__eul_ttae: makeCuratedExample(
    "목소리가 떨려서 긴장이 심해지는 것 같을 때는 천천히 숨을 쉬어요.",
    "Когда мне кажется, что напряжение усиливается и голос начинает дрожать, я медленно дышу."
  ),
  ajida_eojida__geot_gatda_verb_present__myeon_eumyeon: makeCuratedExample(
    "하늘이 갑자기 어두워지는 것 같으면 비가 오기 전에 들어오세요.",
    "Если кажется, что небо вдруг темнеет, зайдите внутрь до дождя."
  ),
  geot_gatda_adj__ji_anta__past_tense__jiman: makeCuratedExample(
    "처음에는 어려운 것 같지 않았지만 막상 해 보니 생각보다 복잡했어요.",
    "Сначала не казалось, что это трудно, но когда я попробовал(а), все оказалось сложнее, чем думалось."
  ),
  geot_gatda_adj__ji_anta__past_tense__eul_ttae: makeCuratedExample(
    "그때는 문제가 큰 것 같지 않았을 때라서 모두가 그냥 넘어갔어요.",
    "Тогда это было время, когда проблема не казалась серьезной, поэтому все просто пропустили ее мимо."
  ),
  geot_gatda_adj__ji_anta__past_tense__myeon_eumyeon: makeCuratedExample(
    "어제처럼 위험한 것 같지 않았으면 저도 같이 갔을 거예요.",
    "Если бы вчера это не казалось таким опасным, я бы тоже пошел(пошла) вместе со всеми."
  ),
  geot_gatda_verb_present__ji_anta__past_tense__jiman: makeCuratedExample(
    "처음에는 비가 오는 것 같지 않았지만 곧 소나기가 쏟아졌어요.",
    "Сначала не казалось, что идет дождь, но вскоре хлынул ливень."
  ),
  geot_gatda_verb_present__ji_anta__past_tense__eul_ttae: makeCuratedExample(
    "그때는 아이가 자는 것 같지 않았을 때라서 조용히 말할 필요가 없었어요.",
    "Тогда это был момент, когда не казалось, что ребенок спит, поэтому не нужно было говорить шепотом."
  ),
  geot_gatda_verb_present__ji_anta__past_tense__myeon_eumyeon: makeCuratedExample(
    "어제처럼 사람이 기다리는 것 같지 않았으면 바로 들어갔을 거예요.",
    "Если бы вчера не казалось, что кто-то ждет, я бы сразу вошел(вошла)."
  ),
  go_sipda__geot_gatda_adj__ji_anta__aseo_eoseo: makeCuratedExample(
    "오늘은 아무도 만나고 싶은 것 같지 않아서 휴대폰도 잠시 꺼 둘게요.",
    "Сегодня, похоже, мне совсем не хочется никого видеть, поэтому я ненадолго выключу телефон."
  ),
  go_sipda__geot_gatda_adj__ji_anta__jiman: makeCuratedExample(
    "민수는 지금 밥을 먹고 싶은 것 같지 않지만 조금 있다가 같이 앉아 있을 거예요.",
    "Похоже, Минсу сейчас не хочет есть, но чуть позже он все равно посидит со всеми за столом."
  ),
  go_sipda__geot_gatda_adj__ji_anta__eul_ttae: makeCuratedExample(
    "제가 누구와도 이야기하고 싶은 것 같지 않을 때는 혼자 산책하면서 마음을 정리해요.",
    "Когда мне кажется, что совсем не хочется ни с кем разговаривать, я иду гулять одна и привожу мысли в порядок."
  ),
  go_sipda__geot_gatda_adj__ji_anta__myeon_eumyeon: makeCuratedExample(
    "저녁에도 아무것도 먹고 싶은 것 같지 않으면 억지로 먹지 말고 쉬세요.",
    "Если и вечером будет казаться, что совсем не хочется ничего есть, не заставляйте себя и отдохните."
  ),
  neuryeogo_hada__geot_gatda_verb_present__ji_anta__aseo_eoseo: makeCuratedExample(
    "민수는 오늘 밖에 나가려고 하는 것 같지 않아서 집으로 바로 돌아갔어요.",
    "Похоже, Минсу сегодня никуда не собирается выходить, поэтому я сразу вернулся(ась) домой."
  ),
  neuryeogo_hada__geot_gatda_verb_present__ji_anta__jiman: makeCuratedExample(
    "지수는 아직 말하려고 하는 것 같지 않지만 곧 마음을 열 거예요.",
    "Похоже, Чису пока не собирается говорить, но скоро она все-таки откроется."
  ),
  neuryeogo_hada__geot_gatda_verb_present__ji_anta__eul_ttae: makeCuratedExample(
    "아이가 더 먹으려고 하는 것 같지 않을 때는 억지로 권하지 않아요.",
    "Когда кажется, что ребенок больше не собирается есть, я не уговариваю его через силу."
  ),
  neuryeogo_hada__geot_gatda_verb_present__ji_anta__myeon_eumyeon: makeCuratedExample(
    "민수가 다시 시도하려고 하는 것 같지 않으면 오늘은 여기서 끝내도 돼요.",
    "Если кажется, что Минсу не собирается пробовать еще раз, на сегодня можно закончить."
  ),
  ajida_eojida__geot_gatda_verb_present__ji_anta__aseo_eoseo: makeCuratedExample(
    "통증이 더 심해지는 것 같지 않아서 오늘은 약만 먹고 쉬려고 해요.",
    "Не похоже, что боль усиливается, поэтому сегодня я просто выпью лекарство и отдохну."
  ),
  ajida_eojida__geot_gatda_verb_present__ji_anta__jiman: makeCuratedExample(
    "날씨가 더 추워지는 것 같지 않지만 밤에는 그래도 조심해야 해요.",
    "Не похоже, что погода станет еще холоднее, но ночью все равно лучше быть осторожнее."
  ),
  ajida_eojida__geot_gatda_verb_present__ji_anta__eul_ttae: makeCuratedExample(
    "상처가 나빠지는 것 같지 않을 때는 집에서 소독하면서 지켜봐요.",
    "Когда не кажется, что рана ухудшается, я просто обрабатываю ее дома и наблюдаю."
  ),
  ajida_eojida__geot_gatda_verb_present__ji_anta__myeon_eumyeon: makeCuratedExample(
    "기침이 더 심해지는 것 같지 않으면 내일까지 조금 더 쉬어 보세요.",
    "Если не кажется, что кашель усиливается, попробуйте просто отдохнуть до завтра."
  )
});
Object.assign(exampleBank.chain, {
  go_sipda__past_tense__jiman: makeCuratedExample(
    "어제는 더 공부하고 싶었지만 너무 피곤해서 일찍 잤어요.",
    "Вчера мне хотелось еще позаниматься, но я слишком устал(а) и лег(ла) спать рано."
  ),
  go_sipda__past_tense__eul_ttae: makeCuratedExample(
    "한국에서 처음 살고 싶었을 때는 모든 것이 새롭고 신기했어요.",
    "Когда я впервые захотел(а) жить в Корее, все казалось новым и удивительным."
  ),
  go_sipda__past_tense__myeon_eumyeon: makeCuratedExample(
    "어제 그렇게 쉬고 싶었으면 오늘은 무리하지 말았어야 했어요.",
    "Если вчера тебе так хотелось отдохнуть, сегодня не стоило перенапрягаться."
  ),
  neuryeogo_hada__past_tense__jiman: makeCuratedExample(
    "주말에는 친구를 만나려고 했지만 갑자기 일이 생겨서 못 갔어요.",
    "На выходных я собирался(ась) встретиться с другом, но внезапно появились дела, и я не смог(ла) пойти."
  ),
  neuryeogo_hada__past_tense__eul_ttae: makeCuratedExample(
    "어제 일찍 자려고 했을 때 친구에게서 전화가 왔어요.",
    "Когда я вчера собирался(ась) лечь спать пораньше, мне позвонил друг."
  ),
  neuryeogo_hada__past_tense__myeon_eumyeon: makeCuratedExample(
    "정말 떠나려고 했으면 왜 마지막에 마음이 바뀌었어요?",
    "Если ты правда собирался(ась) уехать, почему в последний момент передумал(а)?"
  ),
  geot_gatda_adj__past_tense__jiman: makeCuratedExample(
    "처음에는 쉬운 것 같았지만 직접 해 보니 생각보다 복잡했어요.",
    "Сначала казалось, что это легко, но когда я попробовал(а) сам(а), все оказалось сложнее."
  ),
  geot_gatda_adj__past_tense__eul_ttae: makeCuratedExample(
    "문제가 별것 아닌 것 같았을 때는 다들 그냥 웃고 넘겼어요.",
    "Когда проблема казалась пустяковой, все просто посмеялись и пошли дальше."
  ),
  geot_gatda_adj__past_tense__myeon_eumyeon: makeCuratedExample(
    "그때 그렇게 위험한 것 같았으면 저도 같이 말렸을 거예요.",
    "Если бы тогда это казалось настолько опасным, я бы тоже попытался(ась) остановить остальных."
  ),
  geot_gatda_verb_present__past_tense__jiman: makeCuratedExample(
    "처음에는 비가 오는 것 같았지만 밖에 나가 보니 그냥 바람만 세게 불고 있었어요.",
    "Сначала казалось, что идет дождь, но когда я вышел(вышла) наружу, оказалось, что просто дул сильный ветер."
  ),
  geot_gatda_verb_present__past_tense__eul_ttae: makeCuratedExample(
    "누가 따라오는 것 같았을 때는 골목으로 들어가지 않았어요.",
    "Когда мне показалось, что кто-то идет следом, я не стал(а) сворачивать в переулок."
  ),
  geot_gatda_verb_present__past_tense__myeon_eumyeon: makeCuratedExample(
    "정말 누가 보고 있는 것 같았으면 바로 자리를 옮겼을 거예요.",
    "Если бы правда казалось, что кто-то наблюдает, я бы сразу пересел(а) в другое место."
  ),
  ajida_eojida__past_tense__jiman: makeCuratedExample(
    "요즘은 날씨가 따뜻해졌지만 아침저녁으로는 아직 쌀쌀해요.",
    "В последнее время погода потеплела, но утром и вечером все еще прохладно."
  ),
  ajida_eojida__past_tense__eul_ttae: makeCuratedExample(
    "기분이 조금 나아졌을 때 다시 공부를 시작했어요.",
    "Когда мне стало немного лучше, я снова сел(села) заниматься."
  ),
  ajida_eojida__past_tense__myeon_eumyeon: makeCuratedExample(
    "벌써 더 좋아졌으면 오늘은 산책도 할 수 있었을 텐데요.",
    "Если бы мне уже стало лучше, сегодня я смог(ла) бы даже пойти на прогулку."
  )
});
module.exports = {
  grammarList,

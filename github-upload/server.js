...
function sendNotFound(response) {
  sendJson(response, 404, { ok: false, message: "РќРµ РЅР°Р№РґРµРЅРѕ." });
}

function sendMethodNotAllowed(response) {
  sendJson(response, 405, { ok: false, message: "РњРµС‚РѕРґ РЅРµ РїРѕРґРґРµСЂР¶РёРІР°РµС‚СЃСЏ." });
}
  ...
  return new Promise((resolve, reject) => {
    ...
    request.on("end", () => {
      ...
      try {
        ...
        reject(new Error("РќРµ СѓРґР°Р»РѕСЃСЊ РїСЂРѕС‡РёС‚Р°С‚СЊ JSON."));
      }
    });
...
function buildExerciseSetResponse(result, meta = {}) {
  if (!result?.ok) {
    return {
      ...
      message: result?.message || "Р­С‚Рё РіСЂР°РјРјР°С‚РёРєРё РЅРµ СЃРѕРµРґРёРЅСЏСЋС‚СЃСЏ.",
      mode: meta.mode || null
    };
  }
...
async function handleResearchSessionStart(request, response) {
  ...
  if (!participantCode) {
    sendJson(response, 400, {
      ok: false,
      message: "Р’РІРµРґРёС‚Рµ РєРѕРґ СѓС‡Р°СЃС‚РЅРёРєР°."
    });
    return;
  }
...
async function handleResearchEvent(request, response) {
  ...
  if (!body.sessionId || !participantCode || !eventType) {
    sendJson(response, 400, {
      ok: false,
      message: "РќРµ С…РІР°С‚Р°РµС‚ РґР°РЅРЅС‹С… РґР»СЏ Р»РѕРіРёСЂРѕРІР°РЅРёСЏ."
    });
    return;
  }
...
async function handleSelfExercise(request, response) {
  ...
  if (grammarIds.length < MIN_GRAMMAR_SELECTION) {
    sendJson(response, 400, {
      ok: false,
      message: `Р’С‹Р±РµСЂРёС‚Рµ РјРёРЅРёРјСѓРј ${MIN_GRAMMAR_SELECTION} РіСЂР°РјРјР°С‚РёРєРё.`
    });
    return;
  }
...
async function handleFreeExercise(request, response) {
  ...
  if (exercises.length === 0) {
    sendJson(response, 200, {
      ...
      message: "Р­С‚Рё РіСЂР°РјРјР°С‚РёРєРё РЅРµ СЃРѕРµРґРёРЅСЏСЋС‚СЃСЏ."
    });
    return;
  }
...
async function handleCheck(request, response) {
  ...
  if (!exercise?.correctAnswer) {
    sendJson(response, 400, {
      ok: false,
      message: "РќРµРєРѕСЂСЂРµРєС‚РЅРѕРµ СѓРїСЂР°Р¶РЅРµРЅРёРµ."
    });
    return;
  }

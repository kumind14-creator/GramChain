...
function getSafeExample(exercise) {
  ...
  if (exampleUsesExactAnswer(example, exercise)) {
    // AI is preferred, but the exercise must never open without a situation.
    // The local generator is matched to the same word and completed answer.
    return {
      ...example,
      sourceType: example.sourceType === "generated" ? "local_fallback" : example.sourceType
    };
  }
...
async function getBestExampleResponse(exercise) {
  const fallbackExample = getSafeExample(exercise);
  const aiExample = await generateAiExampleForExercise(exercise, {
    fallbackExample
  });

  return aiExample || fallbackExample || null;
}

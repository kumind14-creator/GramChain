const state = {
  ...
  exerciseLogKey: null,
  exerciseContextKey: null,
  exerciseExampleKey: null,
  exerciseExample: null,
  exerciseSituationRu: null,
  exerciseSituationTargetRu: null,
  exerciseSituationEn: null,
  exerciseSituationTargetEn: null,
  exerciseSituationStatus: "idle",
  taskTimes: {},
  completedTasks: {},
...
const MISTAKES_STORAGE_PREFIX = "gramchain_mistakes_";
const MAX_MISTAKE_HISTORY = 200;
const TASK_TEXT = {
  task_1: {
    ru: "Введите код доступа и начните назначенную учебную сессию.",
    en: "Enter the access code and start the assigned learning session."
  },
  task_2: {
    ru: "Начните свою тренировку, выбрав как минимум две грамматические конструкции.",
    en: "Start a Custom practice set by selecting at least two grammar patterns."
  },
  task_3: {
    ru: "Выполните одно упражнение и ознакомьтесь с обратной связью.",
    en: "Complete one exercise and review the feedback."
  },
  task_4: {
    ru: "Просмотрите свои ошибки.",
    en: "Review your mistakes."
  },
  task_5: {
    ru: "Выберите любой режим в свободной тренировке.",
    en: "Choose either mode in Free practice."
  }
};

const WORD_MEANINGS_EN = {
  gada: "go", oda: "come", meokda: "eat", masida: "drink", hada: "do",
  gongbuhada: "study", ilhada: "work", jada: "sleep", ireonada: "get up",
  mannada: "meet", boda: "see / watch", deutda: "listen", ikda: "read",
  sseuda: "write", sada: "buy", salda: "live", joahada: "like",
  silheohada: "dislike", undonghada: "exercise", swida: "rest", tada: "ride",
  yeolda: "open", datda: "close", gidarida: "wait", nolda: "play / hang out",
  mandeulda: "make", keuda: "big", jakda: "small", nopda: "high", natda: "low",
  johta: "good", nappeuda: "bad", jaemiitda: "interesting", jaemieopda: "boring",
  yeppeuda: "pretty", bappeuda: "busy", haengbokhada: "happy", seulpeuda: "sad",
  deopda: "hot", chupda: "cold", tteutteuthada: "warm", siwonhada: "cool / refreshing",
  bissada: "expensive", ssada: "cheap", eoryeopda: "difficult", swipda: "easy",
  gakkapda: "near", meolda: "far", apeuda: "hurt / be ill", dareuda: "different"
};
...
const uiText = {
  ru: {
    siteTitle: "Тренировка соединений корейской грамматики",
    loading: "Загрузка...",
    langLabel: "EN",
    ...
    selfPracticeIntro: "Выберите грамматики, соединения которых хотите потренировать.",
    open: "Открыть",
    quickStart: "Быстрый старт",
    freePractice: "Свободная тренировка",
    freePracticeIntro: "Выберите, хотите потренировать цепочки из 2 или из 3-4 грамматик.",
    chooseGrammars: "Выберите от 2 до 4 грамматик, сочетания которых хотите потренировать.",
    selectedNow: "Сейчас выбрано: {count}",
    backHome: "На главную",
    backToStartPage: "К выбору тренажера и speaking test",
    ...
    tryConnect: "Соедините выбранные грамматики с этим словом. Впишите только получившуюся форму слова, а не полное предложение.",
    exercise: "Упражнение",
    ofTotal: "{current} из {total}",
    word: "Слово",
    situation: "Ситуация:",
    situationLoading: "Подбираем ситуацию для этого упражнения...",
    situationUnavailable: "Не удалось подготовить ситуацию. Попробуйте ещё раз.",
    retrySituation: "Повторить",
    ...
    sessionCompletedTitle: "Сессия завершена",
    sessionCompletedText: "Спасибо. Все данные этой учебной сессии сохранены. Теперь можно закрыть эту страницу.",
    ...
    missingYoRetry: "Цепочка собрана правильно, но ответ дан без вежливого окончания -요.",
    requestError: "Ошибка запроса.",
    retryError: "Произошла ошибка. Попробуйте еще раз."
  },
  en: {
    siteTitle: "Korean Grammar Chaining Trainer",
    loading: "Loading...",
    langLabel: "RU",
    ...
    freePracticeIntro: "Choose whether you want to practice chains of 2 or 3-4 grammar patterns.",
    chooseGrammars: "Choose 2 to 4 grammar patterns whose combinations you want to practise.",
    selectedNow: "Selected now: {count}",
    backHome: "Back to home",
    backToStartPage: "Back to trainer and speaking test",
    ...
    tryConnect: "Connect the selected grammar patterns with this word. Enter only the completed word form, not a full sentence.",
    exercise: "Exercise",
    ofTotal: "{current} of {total}",
    word: "Word",
    situation: "Situation:",
    situationLoading: "Preparing a situation for this exercise...",
    situationUnavailable: "The situation could not be prepared. Please try again.",
    retrySituation: "Try again",
    ...
    sessionCompletedText: "Thank you. The data for this learning session have been saved. You may now close this page.",
    ...
    missingYoRetry: "The chain is correct, but the answer is missing the polite ending -yo.",
...
function getTaskText(taskId) {
  const task = TASK_TEXT[taskId];
  if (!task) {
    return taskId;
  }

  return state.currentLanguage === "ru" ? task.ru : task.en;
}
...
function updateDocumentLanguage() {
  ...
  const heading = document.getElementById("appTitle");
  if (heading) {
...
function renderHighlightedTranslation(text, target) {
  const translation = String(text || "");
  const fragment = String(target || "").trim();
  const index = fragment ? translation.indexOf(fragment) : -1;

  if (index === -1) {
    return escapeHtml(translation);
  }

  return `${escapeHtml(translation.slice(0, index))}<span class="translation-target">${escapeHtml(fragment)}</span>${escapeHtml(translation.slice(index + fragment.length))}`;
}

function getMistakesStorageKey(participantCode) {
  const normalized = String(participantCode || "").trim();
  return normalized ? `${MISTAKES_STORAGE_PREFIX}${normalized}` : null;
}

function loadMistakeHistory(participantCode) {
  const key = getMistakesStorageKey(participantCode);
  if (!key) {
  ...
  try {
    ...
    state.mistakeHistory = Array.isArray(parsed) ? parsed : [];
  } catch (error) {
    state.mistakeHistory = [];
  }
}

function persistMistakeHistory() {
  const key = getMistakesStorageKey(state.participantCode);
  if (!key) {
    return;
  }

  try {
    window.localStorage?.setItem(key, JSON.stringify(state.mistakeHistory));
  } catch (error) {
    // Ignore storage quota or browser privacy errors and keep the in-memory list.
  }
}
...
function getWordMeaning(word) {
  if (state.currentLanguage === "en") {
    return WORD_MEANINGS_EN[word?.id] || word?.meaningEn || "";
  }

  return word?.meaningRu || "";
}
...
function getExampleTranslationTarget(example) {
  return state.currentLanguage === "en"
    ? example?.targetEn || ""
    : example?.targetRu || "";
}
...
function isMissingPoliteYo(submittedAnswer, correctAnswer) {
  const normalizedSubmitted = normalizeAnswerText(submittedAnswer);
  const normalizedCorrect = normalizeAnswerText(correctAnswer);

  if (!normalizedSubmitted || !normalizedCorrect || !normalizedCorrect.endsWith("요")) {
    return false;
  }

  return `${normalizedSubmitted}요` === normalizedCorrect;
}
...
function getResultMessage(result, submittedAnswer = "") {
  ...
  if (result.errorType === "wrong_form") {
    ...
    if (isMissingPoliteYo(submittedAnswer, correctAnswer)) {
      return t("missingYoRetry");
    }
...
function getPoliteInstruction(exercise) {
  ...
  if (exercise.correctAnswer.endsWith("요")) {
    return state.currentLanguage === "en"
      ? "Enter only the completed word form with the grammar patterns, not a full sentence. Use the 아요/어요 form."
      : "Введите только форму слова с грамматиками, не полное предложение. Используйте форму 아요/어요.";
  }

  return state.currentLanguage === "en"
    ? "Enter only the completed word form with the grammar patterns, not a full sentence."
    : "Введите только форму слова с грамматиками, не полное предложение.";
}

function getExplanationText(result, exercise) {
  if (state.currentLanguage !== "en") {
    return result?.explanation?.why || "";
  }

  const order = (exercise?.resolvedOrder || []).map(getGrammarLabel).join(" -> ");
  return order
    ? `The grammar patterns are connected in this order: ${order}.`
    : "The grammar patterns are connected in the required order.";
}
...
function getExerciseLogPayload(exercise) {
  return {
    ...
    practiceSetId: state.currentExerciseSetId,
...
function startPracticeSession() {
  ...
  void logResearchEvent("practice_session_started", { startedAt: startedAtIso });
}
...
async function finishPracticeSession() {
  ...
  await logResearchEvent("practice_session_completed", {
    startedAt: practiceSession.startedAt,
    completedAt,
    practiceDurationMs: Math.max(0, completedAtMs - practiceSession.startedAtMs)
  });
}
...
async function loadExerciseSituation(exercise) {
  const exerciseKey = getExerciseKey(exercise);
  if (!exerciseKey || state.exerciseContextKey === exerciseKey) {
    return;
  }

  state.exerciseContextKey = exerciseKey;
  state.exerciseExampleKey = null;
  state.exerciseExample = null;
  state.exerciseSituationRu = null;
  state.exerciseSituationTargetRu = null;
  state.exerciseSituationEn = null;
  state.exerciseSituationTargetEn = null;
  state.exerciseSituationStatus = "loading";

  try {
    const response = await api("/api/exercise/context", "POST", { exercise });
    if (state.exerciseContextKey !== exerciseKey) {
      return;
    }

    state.exerciseSituationRu = response?.situationRu || null;
    state.exerciseSituationTargetRu = response?.situationTargetRu || null;
    state.exerciseSituationEn = response?.situationEn || null;
    state.exerciseSituationTargetEn = response?.situationTargetEn || null;
    state.exerciseExampleKey = response?.example?.sentence ? exerciseKey : null;
    state.exerciseExample = response?.example || null;
    state.exerciseSituationStatus = response?.situationRu || response?.situationEn ? "ready" : "unavailable";
  } catch (error) {
    console.warn("Could not load exercise situation.", error);
    state.exerciseSituationStatus = "unavailable";
  }

  if (state.currentView === "exercise" && state.exerciseContextKey === exerciseKey) {
    renderExercise();
  }
}
...
function shouldPromptTask2RatingBeforeFollowUp() {
  const result = state.currentResult?.checkResult;
  const submittedAnswer = state.lastSubmittedAnswer.trim();

  return state.currentMode === "self"
    && state.currentExerciseIndex === 0
    && result?.ok === false
    && !shouldRevealAnswer(result, submittedAnswer)
    && state.taskDifficultyRatings.task_2 == null;
}

function shouldTrackFirstSelfExercise() {
  return state.currentMode === "self" && state.currentExerciseIndex === 0;
}

function queuePostResultTaskRatings(nextScreen = "result") {
  if (!shouldTrackFirstSelfExercise() || state.currentView !== "result") {
    return false;
  }

  if (state.taskDifficultyRatings.task_2 == null) {
    return queueTaskDifficultyPrompt("task_2", getTaskText("task_2"), `post-result:${nextScreen}`);
  }

  if (state.taskDifficultyRatings.task_3 == null) {
    return queueTaskDifficultyPrompt("task_3", getTaskText("task_3"), nextScreen);
  }

  return false;
}

function finishTask3Review(nextScreen = "result") {
  if (!state.taskTimes.task_3) {
    return false;
  }

  if (!state.completedTasks.task_3) {
    completeTask("task_3", getTaskText("task_3"), {
  ...
  return queueTaskDifficultyPrompt(
    "task_3",
    getTaskText("task_3"),
    nextScreen
  );
...
function finishTask4Review(nextScreen = "home") {
  if (!state.taskTimes.task_4) {
    return false;
  }

  if (!state.completedTasks.task_4) {
    completeTask("task_4", getTaskText("task_4"), {
      reviewCompleted: true,
      totalMistakes: state.mistakeHistory.length
    });
  }

  if (state.taskDifficultyRatings.task_4 != null) {
    return false;
  }
...
function handleTaskPromptNextScreen(nextScreen = "result") {
  if (nextScreen === "home") {
    renderHome();
    return;
  }

  if (nextScreen === "exercise") {
    renderExercise();
    return;
  }
  ...
  if (nextScreen === "skip-exercise") {
    void skipCurrentExercise();
    return;
  }

  if (nextScreen === "next-exercise") {
    goToNextExercise();
    return;
  }

  if (nextScreen === "finish-set") {
  ...
  appRoot.innerHTML = `
    <section class="screen">
      ...
      <div class="result-box">
        <div class="result-meta"><strong>${t("taskLabel")}</strong> ${escapeHtml(getTaskText(pendingTask.taskId))}</div>
...
async function saveTaskDifficultyRating(rating) {
  ...
  if (String(pendingTask.nextScreen).startsWith("post-result:")) {
    const finalNextScreen = String(pendingTask.nextScreen).slice("post-result:".length) || "result";
    if (pendingTask.taskId === "task_2" && state.taskDifficultyRatings.task_3 == null) {
      queueTaskDifficultyPrompt("task_3", getTaskText("task_3"), finalNextScreen);
      renderTaskDifficultyPrompt();
      return;
    }
    handleTaskPromptNextScreen(finalNextScreen);
    return;
  }

  handleTaskPromptNextScreen(pendingTask.nextScreen);
}
...
function renderParticipantStart() {
  ...
  startTask("task_1", getTaskText("task_1"));
      ...
      <div class="exercise-box participant-form">
        ...
        <p class="subtle-inline-note">
          ${state.currentLanguage === "ru"
            ? "Для записи устных ответов перейдите в Speaking Test."
...
function renderHome() {
  ...
  appRoot.innerHTML = `
    <section class="screen">
      ...
      <div class="button-row">
        <button class="ghost-btn" data-action="back-start">${t("backToStartPage")}</button>
      </div>
    </section>
  `;
}
  ...
  appRoot.querySelectorAll('input[type="checkbox"]').forEach((checkbox) => {
    checkbox.addEventListener("change", (event) => {
      ...
      if (event.target.checked) {
        if (state.selectedGrammarIds.length >= state.maxGrammarSelection) {
          event.target.checked = false;
          alert(state.currentLanguage === "en"
            ? `Choose no more than ${state.maxGrammarSelection} grammar patterns.`
            : `Выберите не больше ${state.maxGrammarSelection} грамматик.`);
          return;
        }
...
function renderExercise() {
  ...
  if (!session?.ok) {
    const connectText = state.currentLanguage === "en" ? session?.messageEn : session?.message;
    const connectMessage = connectText && (state.currentLanguage === "en" || connectText !== "Эти грамматики не соединяются.")
      ? `<p>${escapeHtml(connectText)}</p>`
      : "";
...
function renderResult() {
  ...
  const explanationText = getExplanationText(result, exercise);
...
async function startParticipantSession() {
  ...
  loadMistakeHistory(payload.participantCode);
  completeTask("task_1", getTaskText("task_1"), {
    participantCode: payload.participantCode
  });

  if (queueTaskDifficultyPrompt("task_1", getTaskText("task_1"), "home")) {
    renderTaskDifficultyPrompt();
    return;
  }
...
async function startSelfExercise() {
  if (state.selectedGrammarIds.length < state.minGrammarSelection || state.selectedGrammarIds.length > state.maxGrammarSelection) {
    alert(state.currentLanguage === "en"
      ? `Choose from ${state.minGrammarSelection} to ${state.maxGrammarSelection} grammar patterns.`
      : `Выберите от ${state.minGrammarSelection} до ${state.maxGrammarSelection} грамматик.`);
    return;
  }
  ...
  void startTask("task_2", getTaskText("task_2"), {
...
async function checkCurrentAnswer(answer) {
  ...
  // The situation shown before answering and the feedback example must be one
  // identical generated sentence. Keep the object received for this exercise.
  if (state.exerciseExampleKey === exerciseKey && state.exerciseExample?.sentence) {
    state.currentResult.example = state.exerciseExample;
  }
  ...
  if (state.currentMode === "self" && state.currentExerciseIndex === 0) {
    void completeTask("task_2", getTaskText("task_2"), {
...
function renderCompletionSummary() {
  ...
  const canFinishSession = ["task_1", "task_2", "task_3", "task_4", "task_5"]
    .every((taskId) => state.completedTasks[taskId]);
  ...
  appRoot.innerHTML = `
    <section class="screen">
      ...
      <div class="button-row">
        ...
        ${canFinishSession ? `<button class="primary-btn" data-action="finish-practice-session">${t("finishSession")}</button>` : ""}
      </div>
    </section>
  `;
}

function renderSessionCompleted() {
  setView("session_completed");
  appRoot.innerHTML = `
    <section class="screen">
      <div class="screen-header">
        ${getParticipantBadgeHtml()}
        ${renderLanguageToggle()}
        <h2>${t("sessionCompletedTitle")}</h2>
        <p>${t("sessionCompletedText")}</p>
      </div>
    </section>
  `;
}
...
async function startSpecificChain(grammarIds) {
  ...
  resetSessionUiState();
  state.currentExerciseSetId = createExerciseSetId();
...
function renderExerciseEnhanced() {
  ...
  // Task 2 ends when the first Custom-practice exercise is ready. Ask for
  // its rating before starting Task 3, so the two ratings never appear back-to-back.
  if (shouldTrackFirstSelfExercise() && !state.completedTasks.task_2) {
    completeTask("task_2", getTaskText("task_2"), {
      selectedGrammarIds: exercise?.selectedGrammarIds || state.selectedGrammarIds
    });

    if (state.taskDifficultyRatings.task_2 == null) {
      queueTaskDifficultyPrompt("task_2", getTaskText("task_2"), "exercise");
      renderTaskDifficultyPrompt();
      return;
    }
  }

  if (shouldTrackFirstSelfExercise() && !state.taskTimes.task_3) {
    startTask("task_3", getTaskText("task_3"));
  }
  ...
  if (
    state.currentMode === "free"
    && state.currentExerciseIndex === 0
    && state.taskTimes.task_5
    && !state.completedTasks.task_5
  ) {
    completeTask("task_5", getTaskText("task_5"), {
      difficulty: state.currentDifficulty,
      totalExercises: session.totalExercises || session.exercises.length
    });

    if (queueTaskDifficultyPrompt("task_5", getTaskText("task_5"), "exercise")) {
      renderTaskDifficultyPrompt();
      return;
    }
  }
  ...
  const exerciseKey = getExerciseKey(exercise);
  if (state.exerciseContextKey !== exerciseKey) {
    void loadExerciseSituation(exercise);
  }
  const wordMeaning = getWordMeaning(exercise.word);
  const situation = state.exerciseContextKey === exerciseKey
    ? (state.currentLanguage === "en"
      ? state.exerciseSituationEn || state.exerciseSituationRu
      : state.exerciseSituationRu || state.exerciseSituationEn)
    : null;
  const situationTarget = state.exerciseContextKey === exerciseKey
    ? (state.currentLanguage === "en"
      ? state.exerciseSituationTargetEn || state.exerciseSituationTargetRu
      : state.exerciseSituationTargetRu || state.exerciseSituationTargetEn)
    : null;
  const situationStatus = state.exerciseContextKey === exerciseKey
    ? state.exerciseSituationStatus
    : "idle";
  ...
  appRoot.innerHTML = `
    <section class="screen">
      ...
      <div class="exercise-box">
        ...
        <div class="result-meta"><strong>${t("word")}:</strong> ${escapeHtml(exercise.word.lemma)}${wordMeaning ? ` (${escapeHtml(wordMeaning)})` : ""}</div>
        ${situation ? `<div class="hint-card exercise-situation"><strong>${t("situation")}</strong><div>${renderHighlightedTranslation(situation, situationTarget)}</div></div>` : ""}
        ${situationStatus === "loading" ? `<div class="hint-card exercise-situation"><strong>${t("situation")}</strong><div>${t("situationLoading")}</div></div>` : ""}
        ${situationStatus === "unavailable" ? `<div class="hint-card exercise-situation"><strong>${t("situation")}</strong><div>${t("situationUnavailable")}</div><button class="ghost-btn inline-action" data-action="retry-situation">${t("retrySituation")}</button></div>` : ""}
        <div class="pill-row">
          ${exercise.selectedGrammarIds.map((id) => `<span class="pill">${escapeHtml(getGrammarLabel(id))}</span>`).join("")}
        </div>
        ${politeInstruction ? `<div class="helper">${escapeHtml(politeInstruction)}</div>` : ""}
...
function renderResultEnhanced() {
  ...
  const canShowWhy = revealAnswer && !!explanationText;
  ...
  appRoot.innerHTML = `
    <section class="screen">
      ...
      <div class="result-box">
        ...
        ${canShowWhy && state.showWhy ? `<div class="hint-card"><div>${escapeHtml(explanationText)}</div></div>` : ""}
        ${revealAnswer && example?.sentence ? `
              <div class="example-card">
                <strong>${t("example")}</strong>
                <div>${escapeHtml(example.sentence)}</div>
                ${getExampleTranslation(example) ? `<div class="result-meta">${renderHighlightedTranslation(getExampleTranslation(example), getExampleTranslationTarget(example))}</div>` : ""}
              </div>
            ` : ""}
      </div>
...
async function checkCurrentAnswerEnhanced(answer) {
  ...
  if (shouldTrackFirstSelfExercise() && !state.completedTasks.task_3) {
    completeTask("task_3", getTaskText("task_3"), {
...
function addMistakeRecord(exercise, answer, result) {
  ...
  state.mistakeHistory.unshift(record);
  state.mistakeHistory = state.mistakeHistory.slice(0, MAX_MISTAKE_HISTORY);
  persistMistakeHistory();
}
...
function renderMistakesScreen() {
  ...
  const itemsHtml = state.mistakeHistory.length
    ? state.mistakeHistory
      .map((item) => {
        const wordMeaning = getWordMeaning({ id: item.wordId, meaningRu: item.wordMeaningRu });
        return `
        <article class="result-box">
          <div class="result-meta"><strong>${t("word")}:</strong> ${escapeHtml(item.wordLemma)}${wordMeaning ? ` (${escapeHtml(wordMeaning)})` : ""}</div>
...
renderHome = function renderHomeWithMistakesButton() {
  ...
  if (
    ...
    && !state.completedTasks.task_2
    && !state.taskTimes.task_2
  ) {
    startTask("task_2", getTaskText("task_2"));
  }

  if (
    state.completedTasks.task_4
    && !state.completedTasks.task_5
    && !state.taskTimes.task_5
  ) {
    startTask("task_5", getTaskText("task_5"));
  }
};
...
function bindGlobalActions() {
  appRoot.addEventListener("click", async (event) => {
    ...
    try {
      ...
      if (action === "finish-practice-session") {
      ...
      if (action === "open-mistakes") {
        startTask("task_4", getTaskText("task_4"));
        renderMistakesScreen();
        return;
      }
      ...
      if (action === "back-home") {
        if (state.currentView === "mistakes" && finishTask4Review("home")) {
          renderTaskDifficultyPrompt();
          return;
        }

        if (state.currentView === "result" && queuePostResultTaskRatings("home")) {
          renderTaskDifficultyPrompt();
          return;
        }
      ...
      if (action === "retry-situation") {
        state.exerciseContextKey = null;
        state.exerciseExampleKey = null;
        state.exerciseExample = null;
        state.exerciseSituationStatus = "idle";
        renderExercise();
        return;
      }
      ...
      if (action === "skip-exercise") {
        if (queuePostResultTaskRatings("skip-exercise")) {
          renderTaskDifficultyPrompt();
          return;
        }
      ...
      if (action === "retry-current") {
        if (queuePostResultTaskRatings("retry-current")) {
          renderTaskDifficultyPrompt();
          return;
        }
        state.currentResult = null;
      ...
      if (action === "next-exercise") {
        if (queuePostResultTaskRatings("next-exercise")) {
          renderTaskDifficultyPrompt();
          return;
        }
      ...
      if (action === "finish-set") {
        if (queuePostResultTaskRatings("finish-set")) {
          renderTaskDifficultyPrompt();
          return;
        }
...
async function bootstrap() {
  ...
  state.maxGrammarSelection = payload.constraints.maxGrammarSelection || 4;

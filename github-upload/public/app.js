...
async function loadParticipantMistakes(participantCode) {
  if (!participantCode) {
  ...
  try {
    const payload = await api(`/api/mistakes?participantCode=${encodeURIComponent(participantCode)}`);
    state.mistakeHistory = Array.isArray(payload.items) ? payload.items : [];
  } catch (error) {
    console.error("Mistake history load error:", error);
    state.mistakeHistory = [];
  }
}

async function saveMistakeRecord(record) {
  if (!state.participantCode || !state.researchSessionId || !record) {
    return;
  }

  try {
    await api("/api/mistakes", "POST", {
      sessionId: state.researchSessionId,
      participantCode: state.participantCode,
      mistake: record
    });
  } catch (error) {
    console.error("Mistake history save error:", error);
  }
}
...
async function startParticipantSession() {
  ...
  await loadParticipantMistakes(payload.participantCode);

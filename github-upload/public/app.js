...
function getResultMessage(result, submittedAnswer = "") {
  ...
  if (result.errorType === "wrong_form") {
    return "Порядок подходит, но форма построена с ошибкой.";
  }
...
async function startTask(taskId, label, extra = {}) {
  if (state.taskTimes[taskId] || state.completedTasks[taskId]) {
    return;
  }

  state.taskTimes[taskId] = Date.now();

  await logResearchEvent("task_started", {
    taskId,
    label,
    ...extra
  });
...
async function completeTask(taskId, label, extra = {}) {
  ...
  await logResearchEvent("task_completed", {
    taskId,
    label,
    durationMs,
    status: "completed",
...
function renderParticipantStart() {
  void startTask("task_1", "Enter the access code and start the assigned learning session.");
...
function renderFreeSelection() {
  const cards = state.options.freePractice.difficultyOptions
    .map((item) => {
      const descriptionText = item.id === "hard" ? "3 ??????????." : "2 ??????????.";

      return `
        <article class="choice-card">
          <h3>${escapeHtml(item.label)}</h3>
          <p>${escapeHtml(descriptionText)}</p>
          <button class="primary-btn" data-difficulty="${item.id}">??????</button>
        </article>
      `;
    })
    .join("");
...
async function startParticipantSession() {
  ...
  await completeTask("task_1", "Enter the access code and start the assigned learning session.", {
...
async function startSelfExercise() {
  ...
  await startTask("task_2", "Select at least two grammar patterns and complete the grammar chaining task.", {
    selectedGrammarIds: state.selectedGrammarIds
  });
  await startTask("task_4", "Complete the assigned learning session.", {
    selectedGrammarIds: state.selectedGrammarIds,
    totalExercises: state.currentSession?.totalExercises || 0
  });

  await logResearchEvent("exercise_set_started", {
    mode: "self",
    selectedGrammarIds: state.selectedGrammarIds,
    totalExercises: state.currentSession?.totalExercises || 0
  });
...
async function checkCurrentAnswer(answer) {
  ...
  await logResearchEvent("answer_submitted", {
  ...
  if (state.currentMode === "self" && state.currentExerciseIndex === 0) {
    await completeTask("task_2", "Select at least two grammar patterns and complete the grammar chaining task.", {
  ...
  if (
    ...
    await completeTask("task_4", "Complete the assigned learning session.", {
      totalExercises: state.currentSession?.totalExercises || 0
    });
  }
...
async function repeatCurrentChain() {
  ...
  await logResearchEvent("repeat_chain_clicked", {
    selectedGrammarIds: grammarIds
  });
...
async function skipCurrentExercise() {
  ...
  await logResearchEvent("exercise_skipped", getExerciseLogPayload(getCurrentExercise()));
...
function bindGlobalActions() {
  appRoot.addEventListener("click", async (event) => {
    ...
    try {
      ...
      if (action === "open-self") {
        await logResearchEvent("opened_self_selection");
        renderSelfSelection();
        return;
      }

      if (action === "open-free") {
        await logResearchEvent("opened_free_selection");
        renderFreeSelection();
        return;
      }
      ...
      if (action === "toggle-hint") {
        ...
        if (state.showHint) {
          await logResearchEvent("hint_opened", getExerciseLogPayload(getCurrentExercise()));
        }
        renderExercise();
        return;
      }
      ...
      if (action === "retry-current") {
        ...
        state.showWhy = false;
        state.pendingSkip = false;
        await logResearchEvent("retry_requested", getExerciseLogPayload(getCurrentExercise()));
        renderExercise();
        return;
      }
      ...
      if (action === "toggle-why") {
        ...
        if (state.showWhy) {
          await logResearchEvent("why_opened", getExerciseLogPayload(getCurrentExercise()));
        }
        renderResult();
        return;
      }
      ...
      if (action === "finish-set") {
        await logResearchEvent("exercise_set_completed", {
          mode: state.currentMode,

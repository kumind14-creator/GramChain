...
const TASK_TEXT = {
  ...
  task_2: {
    ru: "Выберите как минимум две грамматические конструкции и выполните одно задание на грамматическую цепочку.",
    en: "Select at least two grammar patterns and start the first grammar chaining task."
  },
  task_3: {
...
function createPracticeSetId() {
  if (window.crypto?.randomUUID) {
    return window.crypto.randomUUID();
  }

  return `practice_${Date.now()}_${Math.random().toString(16).slice(2)}`;
}

function startPracticeSet(mode, details = {}) {
  const startedAtMs = Date.now();
  const startedAt = new Date(startedAtMs).toISOString();

  state.practiceSet = {
    id: createPracticeSetId(),
    mode,
    details,
    startedAtMs,
    startedAt,
  ...
  void logResearchEvent("practice_set_started", {
...
function completePracticeSet() {
  ...
  if (!practiceSet || practiceSet.completed) {
    return;
  }

  const completedAtMs = Date.now();
  const completedAt = new Date(completedAtMs).toISOString();
  practiceSet.completed = true;

  void logResearchEvent("practice_set_completed", {
    practiceSetId: practiceSet.id,
    mode: practiceSet.mode,
    startedAt: practiceSet.startedAt,
    completedAt,
    practiceDurationMs: Math.max(0, completedAtMs - practiceSet.startedAtMs),
    ...practiceSet.details,
...
async function startSpecificChain(grammarIds) {
  ...
  if (state.currentSession?.ok) {
    startPracticeSet("repeat_chain", {

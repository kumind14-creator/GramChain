...
function isPracticeEvent_(body) {
  return body && (
    body.type === "practice_set_started" ||
    body.type === "practice_set_completed"
  );
}
...
function getCanonicalTaskLabel_(taskId) {
  const labels = {
    ...
    task_2: "Task 2: Select at least two grammar patterns and start the first grammar chaining task.",
...
function buildPracticeRecord_(body) {
  ...
  const isCompleted = body.type === "practice_set_completed";
  ...
  return {
    ...
    totalExercises: payload.totalExercises ?? summary.total ?? "",
    correctFirstTry: summary.correctFirstTry ?? "",
    solvedAfterRetry: summary.solvedAfterRetry ?? "",
...
function upsertPracticeRow_(sheet, record) {
  ...
  const values = [
    ...
    record.correctFirstTry,
    record.solvedAfterRetry,
    record.skipped
  ];
...
function doPost(e) {
  try {
    ...
    if (isPracticeEvent_(body)) {
      const practiceSheet = spreadsheet.getSheetByName("practice_sessions") || spreadsheet.insertSheet("practice_sessions");

      ensurePracticeHeaders_(practiceSheet);
      upsertPracticeRow_(practiceSheet, buildPracticeRecord_(body));

      return ContentService
        .createTextOutput(JSON.stringify({ ok: true, target: "practice_sessions" }))
        .setMimeType(ContentService.MimeType.JSON);
    }

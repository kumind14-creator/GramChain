...
const TRANSLATION_POLICY_VERSION = "v8";
...
function validateAiExample(exercise, candidate) {
  ...
  if (candidate.sentenceKo.length < 12 || candidate.sentenceKo.length > 180 || candidate.translationRu.length < 12 || candidate.translationEn.length < 12) {
    return false;
  }
  ...
  if (isBarePastWishOrPlan(exercise, candidate.sentenceKo) || containsKnownTemplateFiller(candidate.sentenceKo)) {
    return false;
  }
...
function buildPromptV2(exercise, fallbackExample) {
  ...
  return [
    ...
    "For example, translate '어제 좋아하는 음악을 듣고 싶었어요.' as 'Вчера хотелось послушать любимую музыку.', not 'Вчера мне хотелось слушать любимую музыку.'",
    ...
    "For -고 싶다 + -(으)ㄴ 것 같다 + -면, prefer a natural third-person context, for example '민수가 좀 더 자고 싶은 것 같으면 깨우지 마세요.' Do not write an unnatural sentence such as '자고 싶은 것 같으면요 바로 연락해 주세요.'",
    "For a target ending in -고 싶었어요 or -려고 했어요, do not write a bare statement such as '어제 문을 닫고 싶었어요.' Add a concrete reason, obstacle, or outcome that makes the sentence natural.",
...
async function generateAiExampleForExercise(exercise, options = {}) {
  ...
  try {
    ...
    if (!aiExample) {
      lastGenerationFailure = "invalid_response";
      return null;
    }
    ...
    lastGenerationFailure = "request_failed";
    return null;
  }
}
